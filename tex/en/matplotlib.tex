...
current Axes'', and will create the \code{Figure} and \code{Axes} if
...
the \code{draw\_text} method, but we haven't gotten around to making
...
\item \textbf{Handle missing data:} The data array may have portions
  ...
  skip over the missing data segments using \code{MOVETO} commands,
...
\item \textbf{Snapping:} Perfectly vertical and horizontal lines can
  ...
  the center of a pixel (see \aosafigref{fig.matplotlib.pixelsnapping}).  The snapping step of the
...
\item \textbf{Simplification:} When plotting really dense plots, many
  ...
  neighboring points are removed (see \aosafigref{fig.matplotlib.pathsimplification}).
