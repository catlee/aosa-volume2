...
at, say, memory address \code{0x658932}.  It then calls a target-side
...
% add a note explaining code<->address mapping
...
disadvantage is that there are many interactions with the target for
each single-step, which for some embedded targets results in
noticeably slow stepping.

% add a display of asm code with source code
...
years in the future.  The \code{configure} shell script is the first
...
something like \code{cat} or \code{diff}, and over time, their
...
to build the tool.  This is what \code{autoconf} is all about, and
...
session, it almost never makes sense to have the executable's methods
...
% add a before/after example from the sources
...
\code{ptrace} would not deliver a signal as soon as it was raised, but
only after running several more instructions.
% dig up a real-life example
...
% need MI example - output of x/16w?
...
% add examples tying into arch changes?
% explain lessons learned from need to re-architect
% ``could not have done perfect design at the outset - incremental is good''
...
But don't try to force dates or timeframes!  For that matter, don't
...
well-defined API, that could be linked into other programs, and the
build process was even changed to build a \code{libgdb.a} as an
intermediate step.  Although the idea came up periodically since then,
it never went further, and as of January 2012 we have abandoned it
entirely and are expunging the pointless bits of code.
...
% add \cite and .bib for main websites at least?
