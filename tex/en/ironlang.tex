...
\code{Parser.y}
file\footnote{\code{Languages/Ruby/Ruby/Compiler/Parser/Parser.y}}, which is a
\code{yacc}-format file that describes the grammar of IronRuby at a high level
using \emph{rules} that describe the grammar. GPPG then takes \code{Parser.y}
...
understand the structure of the language in the \code{Parser.y} file, but it is
...
\code{LightLambdaExpression.Reduce()}, compiled to a \code{DynamicMethod}
...
\aosafigure[250pt]{../images/ironlang/CallSiteClasses.png}{CallSite Class Diagram}{fig.ironlang.callsite}
...
\aosafigure[225pt]{../images/ironlang/CallSites.png}{CallSite Flow Chart}{fig.ironlang.flowchart}
...
\aosafigure[300pt]{../images/ironlang/IDMOP.png}{IDMOP Class Diagram}{fig.ironlang.class}

From a class that implements \code{IDynamicMetaObjectProtocol}, the DLR can get
a \code{DynamicMetaObject} by calling \code{GetMetaObject()}. This
...
object. Then, another binder is called (\code{PythonInvokeBinder.Bind()}) which
calls \code{MetaOldInstance.BindInvoke()}, wrapping the first
