...
% American spelling, no oxford comma.
...
% I can't find any reference to IPK on the internet as a packaging system.
% Closest I can find is ipkg
% Notes: * RPM is an acronym, so changed.
%        * deb is not, so not changed.
...
\code{oe-build-init-env}, some initial configuration files are generated.
...
that the cooker looks for is \code{bblayers.conf}, whose main purpose is to
set up global build time variables, such as directory structure naming
for various rootfs directories, the initial \code{LDFLAGS} to be used during
...
\end{verbatim}
\caption{Portion of \code{bitbake.conf} showing included configuration files}
...
\code{bitbake/bin/bake}, we can begin to see the process BitBake takes as it
...
% "end up hitting swap" is pretty confusing and jargon-y... any suggestions
% on re-wording that?
...
to be run through BitBake's \code{bb.codeparser} to ensure that it's valid
...
% TA: Not sure about use of (R) symbol here either.
...
\code{meta/recipes-core/tasks/task-core-boot.bb} we will see two BitBake
...
...
\end{verbatim}
\caption{Portion of \code{task-core-boot.bb} showing \code{DEPENDS} and \code{RDEPENDS}}
