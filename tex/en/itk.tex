...
of a Data Pipeline architecture. There are two main features that are enabled
by the Data Pipeline: (a) A set of image filters can be concatenated one after
another, composing a processing chain in which it is easy to explore the
effects that changing the paremeters of any of the filters will have on the
final output image. (b) It makes possible to perform memory streaming by
processing only sub-blocks of the image extent, and in this way make possible
to process large images that otherwise would not have fitted in memory.

The simplified representation of an image data pipeline has been illustrated in
figures~\ref{fig.itk.pipeline} and~\ref{fig.itk.brim}. Two main types of
objects were designed to hold the basic structure of the pipeline. They are the
\code{DataObject} and the \code{ProcessObject}. The \code{DataObject} is the
abstraction of classes that carry data. For example, images and geometrical
meshes. The \code{ProcessObject} provide an abstraction for the image filters
and mesh filters. These classes take \code{DataObjects} and perform some type
of algorithmic transformation on them, such as the ones illustrated
in~\ref{fig.itk.brim}.
...
This sequence is illustrated in~\ref{fig.itk.processobjectdataobject}. The same
DataObject may be passed as input to multiple ProcessObjects, as shown in
the Figure for the DataObject constructed by the FileReader at the beginning of
the pipeline. It is also common for some filters to require two DataObjects as
input, as it is the case of the ``Subtract Filter'' illustrated in the right
...
from the Visualization Toolkit (VTK), that was a mature project at the time
that ITK development was in its first steps.
...
between the basic Object, ProcessObject, DataObject, and some of the classes in
the filter family and the data family. In this abstraction, any object that is
expected to be passed as input to a filter, or be produced as output by a
filter, must derive from the DataObject. All filters that produce and consume
data are expected to derive from the ProcessObject. The data negotiations
required to move data through the pipeline are implemented part in the
ProcessObject and part in the DataObject.
...
\aosafigure{../images/itk/BlockFunctionalDiagram.pdf}{IO Factories Dependencies}{fig.itk.io.factories}
