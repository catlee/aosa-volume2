...
reading a \code{DataObject} from disk, for example by using a FileImageReader which is
...
The same \code{DataObject} may be passed as input to multiple \code{ProcessObjects}, as it is
shown in the Figure for the \code{DataObject} constructed by the FileReader at the
beginning of the pipeline. It is also common for some filters to require two
\code{DataObjects} as input, as it is the case of the ``Subtract Filter'' illustrated
