...
FreeRTOS (prounounced ``free-arr-toss'') is an open source real-time
...
use''\footnote{\url{http://www.freertos.org/index.html?http://www.freertos.org/FreeRTOS\_Features.html}}.
...
Windows, Linux, or OS X, but all operating systems share the basic
concepts and goals, and so looking at any OS can be very instructive
and interesting.
...
Embedded systems are computer systems that are designed to do only a
few things, like a TV remote control, in-car GPS system, digital
watch, or a pacemaker.  Embedded systems are typically smaller and
...
of time: Real-time systems guarantee that stuff happens when it's
supposed to.
...
\item{Tasks}: Almost half of FreeRTOS's core code deals with the
  central figure in many operating systems: tasks. A task is a
...
\item{The Hardware Whisperer}: The approximately 9000 lines of code
  that make up the base of FreeRTOS are hardware-independent.  They
  are competely unaware of the underlying hardware or processor
  architecture.  The same code runs no matter what processor FreeRTOS
  is running on, from the humble 8051 to the newest, shiniest ARM
  core.
...
If the hardware-independent FreeRTOS code doesn't talk to the
hardware, then what does?  Why, the hardware-dependent FreeRTOS code,
of course!
...
\aosafigure{../images/freertos/freertos-figures-layers.png}{FreeRTOS software layers}{fig.freertos.layers}
...
Labs 8051, x86, etc.). See the FreeRTOS website for all supported
architectures and compilers.

FreeRTOS is highly configurable by design. FreeRTOS can be built as a
single CPU, bare-bones, stripped down RTOS, supporting only a few
...
\code{vPortEnterCritical()} is actually defined in
...
\code{pxReadyTasksLists[0]} is a list of all ready priority 0 tasks,
...
\aosafigure{../images/freertos/freertos-figures-basic-ready-list.png}{Basic View of FreeRTOS Ready List}{fig.freertos.ready}
...
% FIXME: back-to-back section headings
...
each task. The TCB is defined in \code{tasks.c} like this:
...
created. Higher priority tasks will be given priority over lower
priority tasks. A task's priority can be changed.  If FreeRTOS
...
Interestingly, the TCB structure doesn't have a field that keeps track
...
FreeRTOS what the task is doing. Instead, however, FreeRTOS tracks
...
is implicitly the task's state.

As a task changes from one state to another, FreeRTOS simply moves it
from one list to another, without needing to explicitly maintain a
state variable.
...
treat newly created tasks exactly the same way as tasks that have
already been running for a while; the scheduler doesn't need any
...
is interrupted.  \code{pxPortInitialiseStack()} modifies the stack to
...
remaining registers \code{R1 - R12} get stack space allocated for them
...
scheduling, and also to implement its queues.
...
meaning that the highest priority xItemValue (the largest number) is
at the front of the list and the lowest priority xItemValue (the
smallest number) is at the end of the list.

The \code{pxNext} and \code{pxPrevious} pointers are standard linked
list pointers.

\code{pvOwner} is a pointer to the owner of the list element. This is
...
variable: 0xffff when \code{portTickType} is a 16-bit value and
0xffffffff when \code{portTickType} is a 32-bit value. Other list
...
Most ``traditional'' list accesses you've ever used probably do all of
...
\aosafigure{../images/freertos/freertos-figures-full-ready-list.png}{Full View of FreeRTOS Ready List}{fig.freertos.list}
...
Note that each of \code{struct xListItem} objects is actually the
\code{xGenericListItem} object from the associated TCB.

\aosafigure{../images/freertos/freertos-figures-full-ready-list-2.png}{Full View of FreeRTOS Ready List After a System Timer Tick}{fig.freertos.aftertick}
...
Task A's queue operation is still possible by the time Task A actually
...
run. This way the blocked task doesn't waste CPU cycles that can be
...
guru Michael Barr says it best in his article, ``Mutexes and
Semaphores
...
If you're at all in doubt about semaphores and mutexes please check
...
\code{pcHead} field) to 0 to note that this queue is being used for a
...
if you've never looked at any OS's internals before you now have a
...
debugging, or MPU support. Please check out the source code for more
information about the FreeRTOS architecture.

This chapter also did not discuss how to setup or use
...
discusses exactly that.  It's not free, but it's authoritative and
very well written; I highly recommend it if you're going to use
FreeRTOS.
...
well as very valuable technical review.
