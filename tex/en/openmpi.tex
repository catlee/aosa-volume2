...
\aosafigure{../images/openmpi/open-mpi-mca.pdf}{Framework
  ...
  a {\em base} and one or more components.  This structure is
  replicated in each of the layers shown in
  \aosafigref{fig.openmpi.layers}.  The sample frameworks listed in
  this figure are spread across all three layers: {\tt btl} and {\tt
    coll} are in the OMPI layer, {\tt plm} is in the ORTE layer, and
  {\tt timer} is in the OPAL layer.}{fig.openmpi.mca}
...
\item {\em Open.} The {\em open} call is the initial query function
  ...
  The TCP BTL component {\em open} function mainly initializes some
...
\item {\em Close.} When a framework decides that a component is no
  ...
  The TCP BTL component {\em close} function closes listening sockets
...
\item {\em Query.} This call is a generalized ``do you want to run?''
  ...
  The BTL framework does not use the generic {\em query} function (it
...
\item {\em Parameter registration.} This function is typically the
  ...
  The TCP BTL component {\em register} function creates a variety of
...
As an example of the TCP BTL framework qury functions, the TCP BTL
...
some algorithm rules and other meta data on its component struct.  See
the ``TCP Component Struct (in the BTL Framework)'' code box.
...
\paragraph{Tieing it all together.}
