...
makes it a general dynamic language implementation framework.
...
modules can also be written partially in intep-level code and partially in
app-level code.
...
do some additional transformations. One of these is exception transformation,
where exception handling is rewritten to use return values.
...
the interpreter has a carefully keep track of references to Python objects it is
...
swaped in and out seamlessly. It is trivial to evaluate a garbage collector
...
forgets to inform the GC when an object is no longer in use. The power the GC
...
performed every time an GC managed object is placed in another GC-managed array
...
are known not to escape the trace, meaning they are not passed as as arguments
to an external function call. Structs and constant length arrays can be
virtuals. Virtuals don't have to be allocated, and their data can be stored
directly in registers and on the stack. (This is much like the static malloc
removal phase describe in the paragraph about translation backend
optimizations.) The virtuals optimization strips away the indirection and memory
allocations inefficiencies in the Python interpreter. For example, boxed Python
...
app-level Python language for performance.
...
experience. Its implicit typing takes some time to get used to. Not all Python
language featuress are supported and others are arbitrarily restricted. RPython
is not specified formally anywhere and what the translator accepts can vary from
day to day as RPython is adapted to PyPy's needs. It doesn't help that the
translation often churns for half an hour only to spit out an obscure error.
...
The levels of abstraction in PyPy are not always are clear cut as in
...
generator. The genericness of the JIT generator allows JITs for new languages
and sub-langauges like regular expressions to be added. PyPy is the fastest
Python implementation today because of its JIT generator.
